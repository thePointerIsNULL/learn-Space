...
void CMByteArray::insertElement(size_t pos, const char* dataP, size_t dataLen)
{
	...
	std::memcpy(_data.get() + _size, _data.get() + pos, moveESize);
	std::memcpy(_data.get() + pos, dataP, dataLen);
...
void CMByteArray::reomveElement(size_t pos, size_t len)
{
	...
	std::memcpy(_data.get() + pos, _data.get() + pos + len, moveSize);
	_size -= len;
	std::memset(_data.get() + _size, 0, _capacity - _size);
}
