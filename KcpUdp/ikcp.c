...
const IUINT32 IKCP_ASK_SEND = 1;		// need to send IKCP_CMD_WASK   是否需要发送 IKCP_CMD_WASK 协议类型 [询问对方窗口size]
const IUINT32 IKCP_ASK_TELL = 2;		// need to send IKCP_CMD_WINS   是否需要发送 IKCP_CMD_WINS 协议类型 [告知对方我的窗口size]
...
{
	int size = sizeof(struct IKCPCB);
	// printf("kcpcb:%u\n", sizeof(struct IKCPCB));
...
{
	// printf("%s conv:%u, size:%ld\n", __FUNCTION__, kcp->conv, size);
